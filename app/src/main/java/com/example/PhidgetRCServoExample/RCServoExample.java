...
public class RCServoExample extends Activity {
	...
	DigitalOutput ledColor;
	SeekBar temperatureSimulationBar;
	VoltageRatioInput voltageRatioInput1;
	RCServo rcServo0;
    ...
    public void onCreate(Bundle savedInstanceState) {
		...
		temperatureSimulationBar = (SeekBar) findViewById(R.id.tempSimBar);
		temperatureSimulationBar.setOnSeekBarChangeListener(new targetPositionChangeListener());
...
//			Net.addServer("", "192.168.1.80", 5661, "", 0); // RPi @ Home
//			Net.addServer("", "192.168.1.43", 5661, "", 0);

			//Set addressing parameters to specify which channel to open (if any)
			rcServo0.setDeviceSerialNumber(14475);
//			rcServo0.setIsRemote(true);
			...
			ledColor = new DigitalOutput();
//			ledColor.setChannel(7);
//			ledColor.open(5000);
//			ledColor.setDutyCycle(1);

			voltageRatioInput0 = new VoltageRatioInput();
			voltageRatioInput1 = new VoltageRatioInput();

			//Set addressing parameters to specify which channel to open (if any)
			voltageRatioInput0.setDeviceSerialNumber(39830);
			voltageRatioInput0.setChannel(1);
			voltageRatioInput1.setDeviceSerialNumber(39830);
			voltageRatioInput1.setChannel(1);
//			voltageRatioInput0.setIsHubPortDevice(true);
//			voltageRatioInput0.setHubPort(0);
			voltageRatioInput0.setIsRemote(true);
			voltageRatioInput1.setIsRemote(true);
			...
			voltageRatioInput1.addVoltageRatioChangeListener(new VoltageRatioInputVoltageRatioChangeListener() {
				public void onVoltageRatioChange(VoltageRatioInputVoltageRatioChangeEvent voltageRatioChangeEvent) {
					VoltageRatioInputVoltageRatioChangeEventHandler handler = new VoltageRatioInputVoltageRatioChangeEventHandler(voltageRatioInput0, voltageRatioChangeEvent);
					runOnUiThread(handler);
				}
	...
	class AttachEventHandler3 implements Runnable {
		Phidget ch;

		public AttachEventHandler3(Phidget ch) {
			this.ch = ch;
		}
		...
		public void run() {
			...
			try {
				SeekBar tempSimBar = (SeekBar) findViewById(R.id.tempSimBar);
				tempSimBar.setProgress(tempSimBar.getMax()/2);
	...
	class VoltageRatioInputVoltageRatioChangeEventHandler implements Runnable {
		...
		public void run() {
			...
			try {
				double targetPosition = rcServo0.getTargetPosition()+(voltageRatioChangeEvent.getVoltageRatio()-0.5)*10;
				int channel = -1;
				if (voltageRatioChangeEvent.getVoltageRatio() > 0.5) {
					ledColor.setChannel(7);
                } else {
					ledColor.setChannel(5);
                }
                ledColor.open(5000);
                ledColor.setDutyCycle(1);
